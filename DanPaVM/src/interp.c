...
#define PEEK8() ({    int8_t var = *(uint8_t*)&CODEBUF[vm->pc]; vm->pc += 1; var; })
...
static op_callback instructions[256];
...
DEF_INS(fadd)
{
    ...
    if (VAR_TYPE(var1) != FLOAT || VAR_TYPE(var2) != FLOAT)
        error("fadd of invalid types : %d %d", VAR_TYPE(var1), VAR_TYPE(var2));

    float result = VAR_VAL_FLT(var1) + VAR_VAL_FLT(var2);
...
DEF_INS(fdiv)
{
    ...
    if (VAR_TYPE(var1) != FLOAT || VAR_TYPE(var2) != FLOAT)
        error("fdiv of invalid types");
    if (VAR_VAL_FLT(var2) == 0.0)
        error("division by zero");
...
DEF_INS(fmod)
{
    ...
    if (VAR_TYPE(var1) != FLOAT || VAR_TYPE(var2) != FLOAT)
        error("fmod of invalid types");
    if (VAR_VAL_FLT(var2) == 0.0)
        error("division by zero");

    float result = fmodf(VAR_VAL_FLT(var1), VAR_VAL_FLT(var2));
    PUSH_STACK(MK_FLT(result));
...
DEF_INS(fltl)
{
    ...
    int32_t result = VAR_VAL_FLT(LOC_VARS[var1]) < VAR_VAL_FLT(var2);
