...
static void invalid_syscall(vm_state_t* vm, int syscall_id)
{
    (void)vm;
    printf("bad syscall 0x%x\n", syscall_id);
    vm->stopped = 1;
}

static syscall_callback base_syscalls[256] =
{
    [0] = syscall_printf,
    [1] = read_int,
    [2] = read_str,
...
{
    int replaced = vm->syscalls[number] != NULL;
    vm->syscalls[number] = syscall;
    return replaced;
}
...
void vm_syscall(vm_state_t* vm, int syscall_id)
{
    ...
    if (syscall_id < 256)
    {
        syscall_callback fn = vm->syscalls[syscall_id];
        if (fn == NULL)
        {
            invalid_syscall(vm, syscall_id);
        }
        else
            fn(vm);
    }
...
{
    // setup the base syscalls
    memcpy(vm->syscalls, base_syscalls, sizeof(syscall_callback)*256);
}
