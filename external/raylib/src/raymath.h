...
#if 0
    __m128 the_vec = _mm_set_ps(1, z, y, x);

    __m128 col1 = _mm_load_ps(&mat.m0);
    __m128 col2 = _mm_load_ps(&mat.m1);
    __m128 col3 = _mm_load_ps(&mat.m2);

    __m128 x_lane = _mm_mul_ps(the_vec, col1);
    __m128 y_lane = _mm_mul_ps(the_vec, col2);
    __m128 z_lane = _mm_mul_ps(the_vec, col3);

    result.x = hsum_ps_sse3(x_lane);
    result.y = hsum_ps_sse3(y_lane);
    result.z = hsum_ps_sse3(z_lane);

#else
    // turns out the compiler excells at scheduling scalar floating point operations
...
RMDEF Vector3 Vector3TransformPtr(Vector3 v, Matrix* mat)
{
    Vector3 result = { 0 };
    float x = v.x;
    float y = v.y;
...
// m8 m9 mA mB
// mC mD mE mF
// Transforms a Vector2 by a given row-major Matrix
RMDEF Vector2 Vector2TransformRowMajor(Vector2 v, Matrix* mat)
{
    Vector2 result = { 0 };
    float x = v.x;
    float y = v.y;
...
#if defined(__SSE__)
    __m128 x_lane  = _mm_set1_ps(x);
    __m128 y_lane  = _mm_set1_ps(y);

    __m128 row1 = _mm_load_ps(&mat->m0);
    __m128 row2 = _mm_load_ps(&mat->m1);
    __m128 row4 = _mm_load_ps(&mat->m3);

    x_lane = _mm_mul_ps(x_lane, row1);
    y_lane = _mm_mul_ps(y_lane, row2);

    __m128 vec = _mm_add_ps(row4, _mm_add_ps(x_lane, y_lane));

    _mm_store_ps(&result.x, vec);
#else
    result.x = mat->m0*x + mat->m1*y + mat->m3;
    result.y = mat->m4*x + mat->m5*y + mat->m7;
#endif
...
RMDEF void MatrixPtrTranspose(const Matrix* src, Matrix* dest)
{
#ifdef __SSE__
    __m128 row1 = _mm_load_ps(&src->m0);
    __m128 row2 = _mm_load_ps(&src->m1);
    __m128 row3 = _mm_load_ps(&src->m2);
    __m128 row4 = _mm_load_ps(&src->m3);

    _MM_TRANSPOSE4_PS(row1, row2, row3, row4);

    _mm_store_ps(&dest->m0, row1);
    _mm_store_ps(&dest->m1, row2);
    _mm_store_ps(&dest->m2, row3);
    _mm_store_ps(&dest->m3, row4);
#else
    *dest = MatrixTranspose(*src);
#endif
}
...
#if defined(__SSE__)
static inline void gemm4x4_SSE(float *restrict  A, float *restrict  B, float *restrict C) {
...
#if defined(__SSE__)
    gemm4x4_SSE(&right.m0, &left.m0, &result.m0);
#else
...
RMDEF Matrix MatrixTranslateMultiply(float x, float y, float z, Matrix right)
{
    Matrix result = right;
    result.m12 = x*right.m0 + y*right.m4 + z*right.m8 + right.m12;
    result.m13 = x*right.m1 + y*right.m5 + z*right.m9 + right.m13;
    result.m14 = x*right.m2 + y*right.m6 + z*right.m10 + right.m14;
    result.m15 = x*right.m3 + y*right.m7 + z*right.m11 + right.m15;
