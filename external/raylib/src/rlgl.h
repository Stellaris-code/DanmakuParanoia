...
// non-raylib custom functions
RLAPI void rlCacheTransposedTransform();
RLAPI void rlgl_draw_quad(Rectangle pos_rect, Rectangle tex_rect, Color tint);
RLAPI void rlResetNoTransform();
RLAPI void rlSetNoTransform();
...
RLAPI void rlDisableRenderTexture(unsigned int id);                      // Disable render texture (fbo), return to default framebuffer
...
RLAPI RenderTexture2D rlLoadRenderTexture(int width, int height, int format, int depthBits, bool useDepthTexture, bool multisampled);    // Load a render texture (with color and depth attachments)
...
RLAPI void rlBlitRenderTexture(unsigned int read_fbo, unsigned int draw_fbo, Rectangle src, Rectangle dst);
...
// non-raylib custom functions
void rlCacheTransposedTransform(void)
{
...
void rlTranslatef(float x, float y, float z)
{
    ...
    *RLGL.State.currentMatrix = MatrixTranslateMultiply(x, y, z, *RLGL.State.currentMatrix);
}
...
void rlVertex3f(float x, float y, float z)
{
    ...
    if (RLGL.State.doTransform)
    {
        vec = Vector3TransformPtr(vec, &RLGL.State.transform);
    }
...
{
    DynamicBuffer* data = &RLGL.State.vertexData[RLGL.State.currentBuffer];
    float* ptr = &data->texcoords[2*data->tcCounter];
    ptr[0] = x;
    ptr[1] = y;
    data->tcCounter++;
}

void rlgl_draw_quad(Rectangle pos_rect, Rectangle tex_rect, Color tint)
{
    if (RLGL.State.vertexData[RLGL.State.currentBuffer].vCounter+4 > (MAX_BATCH_ELEMENTS*4))
    {
        TRACELOG(LOG_ERROR, "RLGL: Batch elements overflow (MAX_BATCH_ELEMENTS)");
        return;
    }
    ...
    ptr[1] = tex_rect.y;
    ptr[2] = tex_rect.x;
    ptr[3] = tex_rect.y + tex_rect.height;
    ptr[4] = tex_rect.x + tex_rect.width;
    ptr[5] = tex_rect.y + tex_rect.height;
    ptr[6] = tex_rect.x + tex_rect.width;
    ptr[7] = tex_rect.y;
    data->tcCounter += 4;

    *(uint32_t*)&data->colors[4*data->cCounter] = *(uint32_t*)&tint;
    data->cCounter++;

    float* v_ptr = &data->vertices[3*data->vCounter];

    RLGL.State.vertexData[RLGL.State.currentBuffer].vCounter += 4;
    RLGL.State.draws[RLGL.State.drawsCounter - 1].vertexCount += 4;

    if (RLGL.State.doTransform)
    {
        Vector2 vec1 = {0, 0};
        Vector2 vec2 = {0, 0 + pos_rect.height};
        Vector2 vec3 = {0 + pos_rect.width, 0 + pos_rect.height};
        Vector2 vec4 = {0 + pos_rect.width, 0};

        Matrix transpose;
        MatrixPtrTranspose(&RLGL.State.transform, &transpose);

        vec1 = Vector2TransformRowMajor(vec1, &transpose);
        vec2 = Vector2TransformRowMajor(vec2, &transpose);
        vec3 = Vector2TransformRowMajor(vec3, &transpose);
        vec4 = Vector2TransformRowMajor(vec4, &transpose);

        v_ptr[0] = vec1.x + pos_rect.x;
        v_ptr[1] = vec1.y + pos_rect.y;
        v_ptr[2] = 0.0f;
        v_ptr[3] = vec2.x + pos_rect.x;
        v_ptr[4] = vec2.y + pos_rect.y;
        v_ptr[5] = 0.0f;
        v_ptr[6] = vec3.x + pos_rect.x;
        v_ptr[7] = vec3.y + pos_rect.y;
        v_ptr[8] = 0.0f;
        v_ptr[9] = vec4.x + pos_rect.x;
        v_ptr[10] = vec4.y + pos_rect.y;
        v_ptr[11] = 0.0f;
    }
    else
    {
        ...
        v_ptr[2] = 0.0f;
        v_ptr[3] = pos_rect.x;
        v_ptr[4] = pos_rect.y + pos_rect.height;
        v_ptr[5] = 0.0f;
        v_ptr[6] = pos_rect.x + pos_rect.width;
        v_ptr[7] = pos_rect.y + pos_rect.height;
        v_ptr[8] = 0.0f;
        v_ptr[9] = pos_rect.x + pos_rect.width;
        v_ptr[10] = pos_rect.y;
        v_ptr[11] = 0.0f;
    }
}
...
void rlDisableRenderTexture(unsigned int id)
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
...
void rlBlitRenderTexture(unsigned int read_fbo, unsigned int draw_fbo, Rectangle src, Rectangle dst)
{
    rlglDraw();
    ...
    glBlitFramebuffer(src.x, src.y, src.width, src.height,
                      dst.x, dst.y, dst.width, dst.height, GL_COLOR_BUFFER_BIT, GL_LINEAR);
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
}
...
void rlDeleteRenderTextures(RenderTexture2D target)
{
    ...
    if (target.multisampling.id > 0) glDeleteTextures(1, &target.multisampling.id);
...
RenderTexture2D rlLoadRenderTexture(int width, int height, int format, int depthBits, bool useDepthTexture, bool multisampled)
{
    ...
    if (multisampled)
    {
        glGenTextures( 1, &target.multisampling.id);
        glBindTexture( GL_TEXTURE_2D_MULTISAMPLE, target.multisampling.id );
        glTexImage2DMultisample( GL_TEXTURE_2D_MULTISAMPLE, 4, GL_RGBA8, width, height, false );
        target.multisampling.width = width;
        target.multisampling.height = height;
        target.multisampling.format = UNCOMPRESSED_R8G8B8A8;
        target.multisampling.mipmaps = 1;
    }
    ...
    // Multisampling?
    if (multisampled)
        target.texture = target.multisampling;
    ...
    if (depthBits > 0)
    {
        if (multisampled)
        {
            ...
            glBindTexture( GL_TEXTURE_2D_MULTISAMPLE, target.depth.id );
    ...
    if (!multisampled)
        ...
        rlRenderTextureAttach(target, target.texture.id, 2);      // COLOR_MULTISAMPLE attachment
...
void rlRenderTextureAttach(RenderTexture2D target, unsigned int id, int attachType)
{
    ...
    else if (attachType == 2) // multisample
    {
...
bool rlRenderTextureComplete(RenderTexture target)
{
    ...
    if (status != GL_FRAMEBUFFER_COMPLETE)
    {
        switch (status)
        {
            ...
            default:TRACELOG(LOG_WARNING, "FBO: [ID %i] Something went wrong, code : 0x%x", target.id, status); break;
        }
    }
...
void InitVrSimulator(void)
{
    ...
    RLGL.Vr.stereoFbo = rlLoadRenderTexture(RLGL.State.framebufferWidth, RLGL.State.framebufferHeight, UNCOMPRESSED_R8G8B8A8, 24, false, false);
...
void EndVrDrawing(void)
{
    ...
    if (RLGL.Vr.simulatorReady)
    {
        ...
        rlDisableRenderTexture(0);       // Unbind current framebuffer
