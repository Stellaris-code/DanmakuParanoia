...
static Model LoadOBJ(const char *fileName)
{
    ...
    if (fileData != NULL)
    {
        ...
        for (unsigned int af = 0; af < attrib.num_faces; af++)
        {
            ...
            //printf("vcount : %d (%d | %d) / %d; af %d, material %d\n",vCount[mm], mm, attrib.material_ids[af], model.meshes[mm].vertexCount*3, af, materialCount);
            if (vCount[mm] < model.meshes[mm].vertexCount*3)
            {
