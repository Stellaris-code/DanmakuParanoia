...
// Default internal render batch limits
        ...
        #define DEFAULT_BATCH_BUFFER_ELEMENTS   8192*4
    #endif
    #if defined(GRAPHICS_API_OPENGL_ES2)
...
RLAPI void rlRotateSpriteRadf(float angleRad); // custom function for optimizing
RLAPI void rlScalef(float x, float y, float z);       // Multiply the current matrix by a scaling matrix
RLAPI void rlMultMatrixf(float *matf);                // Multiply the current matrix by another matrix
RLAPI void rlLoadMatrixf(float *matf);
...
RLAPI void rlgl_draw_quad(Rectangle pos_rect, Vector2 origin, float angle, Rectangle tex_rect, Color tint);
...
RLAPI RenderBatch* rlCurrentRenderBatch();
...
RLAPI void rlDrawVertexArrayElementsInt(int offset, int count, void *buffer);
...
void rlLoadMatrixf(float *matf) { glLoadIdentity(); glMultMatrixf(matf); }
#endif
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
...
// Multiply the current matrix by a rotation matrix
void rlRotateSpriteRadf(float angleRad)
{
    Matrix matRotation = MatrixRotateZ(-angleRad);
...
static void fast_sincos(float angle, float *sin, float *cos){
    ...
    //angle=angle-floorf(angle*1.0/twopi)*twopi;
...
void rlgl_draw_quad(Rectangle pos_rect, Vector2 origin, float angle, Rectangle tex_rect, Color tint)
{
    /*
    if (RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer].vCounter+4 >= (RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer].elementsCount*4))
    {
    ...
    */
    ...
    VertexBuffer* data = &RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer];
    ...
    *(uint32_t*)&data->colors[4*data->cCounter] = *(uint32_t*)&tint;
    ...
    if (angle != 0.0f)
    {
        float cosres;
        float sinres;
        //sincosf(angle, &sinres, &cosres);
        fast_sincos(angle, &sinres, &cosres);

        Vector2 vec1 = {-origin.x, -origin.y};
        Vector2 vec2 = {-origin.x, -origin.y + pos_rect.height};
        Vector2 vec3 = {-origin.x + pos_rect.width, -origin.y + pos_rect.height};
        Vector2 vec4 = {-origin.x + pos_rect.width, -origin.y};
        vec1 = (Vector2){vec1.x*cosres - vec1.y*sinres, vec1.x*sinres + vec1.y*cosres};
        vec3 = (Vector2){vec3.x*cosres - vec3.y*sinres, vec3.x*sinres + vec3.y*cosres};
        vec2 = (Vector2){vec2.x*cosres - vec2.y*sinres, vec2.x*sinres + vec2.y*cosres};
        vec4 = (Vector2){vec4.x*cosres - vec4.y*sinres, vec4.x*sinres + vec4.y*cosres};
...
void rlDrawVertexArrayElementsInt(int offset, int count, void *buffer)
{
