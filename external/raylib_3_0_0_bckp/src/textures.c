...
RenderTexture2D LoadRenderTexture(int width, int height, bool multisampled)
{
    RenderTexture2D target = rlLoadRenderTexture(width, height, UNCOMPRESSED_R8G8B8A8, 24, false, multisampled);
...
TextureCubemap LoadTextureCubemap(Image image, int layoutType, Image* faces_image)
{
    ...
    if (layoutType != CUBEMAP_AUTO_DETECT)
    {
        ...
        /*
        ImageDraw(&faces, image, faceRecs[0], (Rectangle){ 0, size*0, size, size }, WHITE);
        ImageDraw(&faces, image, faceRecs[1], (Rectangle){ 0, size*1, size, size }, WHITE);
        ImageDraw(&faces, image, faceRecs[2], (Rectangle){ 0, size*2, size, size }, WHITE);
        ImageDraw(&faces, image, faceRecs[3], (Rectangle){ 0, size*3, size, size }, WHITE);
        ImageDraw(&faces, image, faceRecs[4], (Rectangle){ 0, size*4, size, size }, WHITE);
        ImageDraw(&faces, image, faceRecs[5], (Rectangle){ 0, size*5, size, size }, WHITE);
        */
...
// NOTE POUR LES ISSUES à report:
// if ((src.width != (int)srcRec.width) && (src.height != (int)srcRec.height)) ImageCrop(&srcCopy, srcRec);
// doit être
// if ((src.width != (int)srcRec.width) || (src.height != (int)srcRec.height)) ImageCrop(&srcCopy, srcRec);
// ET
// dans LoadTextureCubemap : cubemap.width n'est pas set si CUBEMAP_AUTO_DETECT n'est pas choisi
...
void ImageDraw(Image *dst, Image src, Rectangle srcRec, Rectangle dstRec, Color tint)
{
    ...
    for (int j = (int)dstRec.y; j < (int)(dstRec.y + dstRec.height); j++)
    {
        for (int i = (int)dstRec.x; i < (int)(dstRec.x + dstRec.width); i++)
        {
            ...
            //dstPixels[j*(int)dst->width + i] = srcPixels[(j - (int)dstRec.y)*(int)dstRec.width + (i - (int)dstRec.x)];
            //dstPixels[j*512 + i] = srcPixels[j*(512*6) + i];
    ...
    *dst = LoadImageEx(dstPixels, (int)dst->width, (int)dst->height);
    //*dst  = LoadImageEx(srcPixels, (int)srcCopy.width, (int)srcCopy.height);
