...
#ifndef RL_MALLOC
    #define RL_MALLOC(sz)       danpa_alloc(sz)
#endif
#ifndef RL_CALLOC
    #define RL_CALLOC(n,sz)     danpa_calloc(n,sz)
#endif
#ifndef RL_REALLOC
    #define RL_REALLOC(ptr,sz)  danpa_realloc(ptr,sz)
#endif
#ifndef RL_FREE
    #define RL_FREE(ptr)        danpa_free(ptr)
#endif
...
typedef __attribute__((aligned(16))) struct Matrix {
    float m0, m4, m8, m12;
    float m1, m5, m9, m13;
    float m2, m6, m10, m14;
    float m3, m7, m11, m15;
...
typedef struct RenderTexture2D {
    ...
    Texture2D multisampling; // Multisampling texture
...
RLAPI void EndTextureMode(RenderTexture2D target);                                  // Ends drawing to render texture
RLAPI void BlitRenderTexture(RenderTexture2D target, Rectangle src, Rectangle dst);
...
RLAPI TextureCubemap LoadTextureCubemap(Image image, int layoutType, Image* face_ptr);                                    // Load cubemap from image, multiple image cubemap layouts supported
RLAPI RenderTexture2D LoadRenderTexture(int width, int height, bool multisampled);                                          // Load texture for rendering (framebuffer)
...
RLAPI void DrawModelExXYZ(Model model, Vector3 position, Vector3 rotationAngles, Vector3 scale, Color tint);
